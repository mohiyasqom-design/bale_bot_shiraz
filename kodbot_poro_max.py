...
def handle_view_plan(query: dict, plan_key: str):
    ...
    text += f"📋 نگهدارید لاگ: {plan['log_retention_days']} روز\n"
        ...
        with get_db() as conn:
            ...
            if plan_key == "basic":
                ...
                if basic_count >= 3:
                    send_message(
                        user_id,
                        "⚠️ *توجه:* شما ۳ بار اشتراک پایه خریدارید کرده‌اید.\n"
...
def handle_admin_restart_bot(query: dict, bot_id: int):
    """راه‌اندازی مجدد ربات شماسط ادمین"""
    admin_id = query["from"]["id"]
    if not is_admin(admin_id):
        return
    with get_db() as conn:
...
def handle_subscription_mgmt(query: dict):
    ...
    with get_db() as conn:
        for plan_key, plan_data in PLANS.items():
            ...
            revenue = revenue_row["s"] or 0
            text += f"*{plan_data['name']}*\n  خریدارید شده: {total} | فعال: {active} | درآمد: {revenue:,}T\n\n"
...
def handle_plan_subscribers(query: dict, plan_key: str, page: int = 0):
    """نمایش لیست کاربرانی که یک پلن خاص را خریدارید کرده‌اند"""
    admin_id = query["from"]["id"]
    chat_id = query["message"]["chat"]["id"]
    msg_id = query["message"]["message_id"]
    ...
    if not subscribers:
        edit_message(chat_id, msg_id, f"هیچ خریدارید برای پلن {plan_name} یافت نشد.",
...
def handle_web_delete_project(query: dict):
    ...
    edit_message(query["message"]["chat"]["id"],
        ...
        reply_markup=inline_keyboard([
            [("✅ بله، حذف کن", f"web_delete_confirm:{project_id}"), ("❌ انصراف", "web_edit_project")]
        ])
    )
...
def handle_api_key_label_input(message: dict):
    ...
    with get_db() as conn:
        # تاریخ انقضای کلید = تاریخ انقضای پلن کاربر
        row = conn.execute(
            "SELECT plan_expires_at FROM users WHERE user_id=?",
            (user_id,)
        ).fetchone()
        expires_at = row["plan_expires_at"] if row and row["plan_expires_at"] else None
...
def main_menu(user_id: int) -> dict:
    buttons = [
        ...
        [("🔑 API (به زودی)", "api_keys_menu"), ("📞 ارتباط با ادمین", "contact_team_menu")],
...
# ── توابع اضافی (کد مرده — برای بررسی) ──
