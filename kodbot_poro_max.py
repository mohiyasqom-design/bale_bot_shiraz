...
def handle_pay_now(query: dict):
    ...
    # کد تخفیف را در payload کدگذاری می‌کنیم تا در handle_successful_payment
    # (فقط پس از پرداخت واقعی) با apply_discount_code مصرف شود.
...
def handle_successful_payment(message: dict):
    ...
    if len(parts) >= 3 and parts[0] == "sub":
        ...
        # کد تخفیف (در صورت وجود) اینجا مصرف می‌شود — تنها پس از پرداخت واقعاً
        # موفق. شناسه کد در payload توسط سرور در handle_pay_now نوشته شده بود؛
        # کاربر امکان تغییر محتوای payload تاییدشده توسط درگاه را ندارد.
        if len(parts) >= 6:
            try:
